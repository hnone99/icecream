...
window.onload = () => {
  ...
  let setLanguage = (lang) => {
    ...
    if(typeof setLanguageAfter == "function") {
      setLanguageAfter();
    }
  };
...
var com_github_culmat_jsTreeTable =  (function(){
  ...
  function treeTable(img,table){
    ...
    var imgExpand = img+"common/square-plus.png"
    var imgCollapse = img+"common/square-minus.png"
    $("tr[data-tt-level]", table).each(function(index, tr) {
      var level = $(tr).attr('data-tt-level')
      var td = $("td",tr).first()
      if(tr.trChildren.length>0){
        td.prepend($('<span class="icon-tree" style="cursor:pointer; width: 16px; height: 16px; float: right; display: inline-block;"><img id="state" style=" width:100%; height:100%;" src="'+imgCollapse+'"/></span>'))
// td.prepend($('<span style="display: none"><img id="state" style=" width:100%; height:100%;" src="'+imgCollapse+'"/></span>'))
      } else {
        td.prepend($('<span style="display: none" /></span>'))
      }
