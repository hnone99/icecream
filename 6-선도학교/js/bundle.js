...
$(function(){
	...
	$("body").on("click", ".customSelect li, .customSelect2 li, .custom_select li", function(e){
		...
		//목록 불러오기 function이 있는 경우 목록 조회 실행
	});
...
$(document).ready(function(){
  ...
  let setLanguage = function (lang, target) {

		let textList = null;
		if(target){

			textList = Array.prototype.slice.call(target.querySelectorAll('[data-text]'));
		}
		else{

			textList = Array.prototype.slice.call(document.querySelectorAll('[data-text]'));
		}
    textList.map( v = function() {
    ...
    placeholderList.map(v =  function() {
      v.placeholder = langData[lang][v.dataset.placeholder]
		});
		//console("fnOnLoaded ? " + fnOnLoaded);
		try{
